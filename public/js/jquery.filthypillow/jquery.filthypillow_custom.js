  ...
  FilthyPillow.prototype = {
    template: '<div class="fp-container">' +
                '<div class="fp-calendar">' +
                  '<span class="fp-year fp-option"></span>-<span class="fp-month fp-option"></span>-<span class="fp-day fp-option"></span>' +//custom
                '</div>' +
                '<div class="fp-clock">' +
                  '<span class="fp-hour fp-option"></span>:<span class="fp-minute fp-option"></span>' +
                  ':<span class="fp-second fp-option"></span>' +//custum
                '</div>' +
    ...
    setup: function( ) {
      ...
      this.$year = this.$calendar.find( ".fp-year" );//add
      ...
      this.$second = this.$clock.find( ".fp-second" );//add
...
//console.info( "step:"+step+","+"digit:"+digit+","+"value:"+value )
...
//console.info( "this.isActiveLeadingZero: " + this.isActiveLeadingZero );
...
//console.info( "precedingDigit: " + precedingDigit );
	  ...
	  //add 20150703
      if( step === "year" ){
    	if( digit >= 2 && digit <= 4 && !this.isActiveLeadingZero ) {
    	  precedingDigit = this.dateTime.get( step );
  	      fakeValue = parseInt( precedingDigit + "" + value, 10 );
    	}
    	else{
...
//console.info( "this.isActiveLeadingZero: " + this.isActiveLeadingZero );
//console.info("fakeValue1:"+fakeValue);

      fakeValue = fakeValue || value;
//console.info("fakeValue2:"+fakeValue);

      if( step === "hour" ) //this is retain the current meridiem
        //fakeValue = this.to24Hour( fakeValue );//co 20150703
    	fakeValue = fakeValue;//modify 20150703
      else
        fakeValue = this.formatToMoment( step, fakeValue );
//console.info("fakeValue3:"+fakeValue);

//console.info("this.currentDigit:"+this.currentDigit);
...
//console.info("this.currentDigit:"+this.currentDigit);
//console.info("this.dateTime.format( A ):"+this.dateTime.format( "A" ));

      if( step != "year" ) {//add 20150703
	      if( this.currentDigit === 2 )
	        moveNext = true;
	        ...
	        moveNext = true;
	      else if( step === "hour" && value > 2 )
	        moveNext = true;
	      else if( step === "minute" && value > 5 )
	        moveNext = true;
      }else{//add 20150703
	      if( this.currentDigit === 4 )//add 20150703
	        moveNext = true;//add 20150703
      }//add 20150703
...
//console.info(keyCode);
      ...
      if(this.currentStep != "year"){//add 20150703
          if( this.currentDigit === 2 )
              this.currentDigit = 0;
      }
    //add 20150703
    ...
    moveDown: function( ) {
      if( this.currentStep === "meridiem" ) {
        ...
        this.changeDateTimeUnit( this.currentStep, -1 );//-15 -> -1
  ...
  function Calendar( $element, options ) {
    ...
    this.$year = this.$container.find( ".fp-cal-year" );//add
