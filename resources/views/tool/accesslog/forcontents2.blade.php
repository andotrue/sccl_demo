...
<div class="row">
	...
	<li class=""><a href="/tool/accesslog/forcontents/1">日次サマリー</a></li>
	<li class="active"><a href="/tool/accesslog/forcontents/2">ページ毎</a></li>
</ul>
...
<div class="col-md-10">
		<!-- 検索部分 -->
		<form action="/tool/accesslog/forcontents/2" method="POST">
		...
		@if($accesslogs->count())
			<!-- Paganation -->
			{{ $accesslogs->appends(['target_month'=>$target_month, 'store_id'=>$store_id])->links() }}
			...
			<table id="table_id" class="table table-striped table-bordered table-hover">
				...
				<tbody class="jquery-ui-sortable-table">
				    ...
				    @foreach($accesslogs as $accesslog)
				        <tr>
							<td>{{$i}}</a></td>
				            <td><a href="{{$accesslog->pagePath}}" target="_blank">{{$accesslogs_pagetitle[$accesslog->pagePath]->pageTitle or ""}}</a></td>
							<td>{{$accesslog->pagePath}}</a></td>
							<td>{{$accesslogs_sesssion[$accesslog->pagePath][0] or 0}}</td>
